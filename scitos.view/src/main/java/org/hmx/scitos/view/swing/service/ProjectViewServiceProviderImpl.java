...
public class ProjectViewServiceProviderImpl implements IProjectViewServiceRegistry, IProjectViewServiceProvider {
    ...
    public final <P extends IViewProject<M>, M extends IModel<M>> void registerViewService(final Class<P> projectClass, final Class<M> modelClass,
            final IProjectViewService<P, M> viewService) {
    ...
    public ContextMenuBuilder createContextMenu(final IViewProject<?> project, final Object element) {
        ...
        ContextMenuBuilder result = service.createContextMenu(project, element);
        if (project == element || project.getModelObject() == element) {
            if (result == null) {
                result = new ContextMenuBuilder(project.getLabel(element));
            } else {
                result.addSeparator();
            }
            result.addItem(Message.PROJECT_CLOSE.get(), new CMenuItemAction() {
    ...
    @Override
    public AbstractProjectView<?, ? extends IModel<?>> createProjectView(final IViewProject<?> project) {
        final IProjectViewService<?, ?> service = this.getServiceByProject(project);
