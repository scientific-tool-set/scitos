...
td:nth-child(1) { padding-left: 0.5em; }
td:not(:nth-child(1)) {
	text-align: right;
	padding-right: 0.5em;
}
			...
			<xsl:when test="@color">
				...
				<xsl:call-template name="decimal-to-hex">
					<xsl:with-param name="decimal"
						select="substring-before($rgb-values,',')" />
				</xsl:call-template>
				<xsl:variable name="gb-values" select="substring-after($rgb-values,',')" />
				<xsl:call-template name="decimal-to-hex">
					<xsl:with-param name="decimal"
						select="substring-before($gb-values,',')" />
				</xsl:call-template>
				<xsl:call-template name="decimal-to-hex">
					<xsl:with-param name="decimal"
						select="substring-after($gb-values,',')" />
				</xsl:call-template>
			</xsl:when>
			<xsl:otherwise>
